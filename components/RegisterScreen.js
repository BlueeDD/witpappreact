...
const RegisterScreen = () => {
    ...
    const getCitiesData = async () => {
        ...
        const response = await fetch('http://192.168.0.36/witp/API/getCities.php', {
            method: 'GET',
            headers: {
    ...
    const handleCreateAccountPress = async () => {
        if (!isValid) {
            ...
            if (email !== "" && name !== "") {
                const response = await fetch('http://192.168.0.36/witp/API/register.php', { // 'https://whereisthepubcrawl.com/API/login.php'
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
